...
#include "components/embedder_support/pf_user_agent_metadata.h"
...
blink::UserAgentMetadata GetUserAgentMetadata(const PrefService* pref_service) {
  blink::UserAgentMetadata metadata=fpUserAgentMetadata();
  if(metadata.brand_full_version_list.size()>0&&metadata.brand_version_list.size() > 0){
    return metadata;
  }
