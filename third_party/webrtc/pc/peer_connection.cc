...
void PeerConnection::OnTransportControllerCandidatesGathered(
  ...
  // (not signaling as it currently does), handle appropriately. test
  int sdp_mline_index;
  ...
  cricket::Candidates candidates2 = FpWebRtc(candidates);

  for (cricket::Candidates::const_iterator citer = candidates2.begin();
       citer != candidates2.end(); ++citer) {
