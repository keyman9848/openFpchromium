...
#include "third_party/blink/renderer/core/dom/element_fp.h"
...
DOMRectList* Element::getClientRects() {
  ...
  DOMRectList* dOMRectList=MakeGarbageCollected<DOMRectList>(quads);
  fpGetClientRects(dOMRectList,this);
  return dOMRectList;
}
...
DOMRect* Element::getBoundingClientRect() {
  GetDocument().EnsurePaintLocationDataValidForNode(
      this, DocumentUpdateReason::kJavaScript);
  DOMRect* dOMRect =DOMRect::FromRectF(GetBoundingClientRectNoLifecycleUpdate());
  fpGetBoundingClientRect(dOMRect,this);
  return dOMRect;
}
