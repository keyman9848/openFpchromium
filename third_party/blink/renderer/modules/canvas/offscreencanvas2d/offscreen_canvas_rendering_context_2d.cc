...
void OffscreenCanvasRenderingContext2D::clearRect(double x,
                                         double y,
                                         double width,
                                         double height) {
  BaseRenderingContext2D::clearRect(x, y, width, height);
  blink::fp::fpFullCoverage(this,x,y, width, height,0,0,Width(),Height());
}

void OffscreenCanvasRenderingContext2D::fill(const String& winding_rule_string) {
    bool flag= blink::fp::fpCanvas(this,this->GetState(), *this, true);
    BaseRenderingContext2D::fill(winding_rule_string);
    this->setFpCanvasTrue(flag);
}

void OffscreenCanvasRenderingContext2D::fill(Path2D* dom_path,
                                  const String& winding_rule_string) {
    bool flag = blink::fp::fpCanvas(this,this->GetState(), *dom_path, true);
    BaseRenderingContext2D::fill(dom_path, winding_rule_string);
    this->setFpCanvasTrue(flag);
 
}

void OffscreenCanvasRenderingContext2D::stroke() {
    bool flag = blink::fp::fpCanvas(this,this->GetState(), *this, false);
    BaseRenderingContext2D::stroke();
    this->setFpCanvasTrue(flag);
}

void OffscreenCanvasRenderingContext2D::stroke(Path2D* dom_path) {
    bool flag = blink::fp::fpCanvas(this,this->GetState(), *dom_path, false);
    BaseRenderingContext2D::stroke(dom_path);
    this->setFpCanvasTrue(flag);
}



void OffscreenCanvasRenderingContext2D::fillRect(double x,
    double y,
    double width,
    double height) {
    ...
    if (!GetOrCreatePaintCanvas())
        return;
    bool flag = blink::fp::fpCanvas(this, this->GetState(), true);
    BaseRenderingContext2D::fillRect(x,y, width, height);
    this->setFpCanvasTrue(flag);
    blink::fp::fpFullCoverage(this,x,y, width, height,0,0,Width(),Height());
}

void OffscreenCanvasRenderingContext2D::strokeRect(double x,
    double y,
    double width,
    double height) {
    if (!(std::isfinite(x) && std::isfinite(y) && std::isfinite(width) &&
    ...
    if (!GetOrCreatePaintCanvas())
        return;
    bool flag = blink::fp::fpCanvas(this, this->GetState(), true);
    BaseRenderingContext2D::strokeRect(x, y, width, height);
    this->setFpCanvasTrue(flag);
}
ImageData* OffscreenCanvasRenderingContext2D::getImageData(
    int sx,
    int sy,
    int sw,
    int sh,
    ExceptionState& exception_state) {
  ImageData* imageData = BaseRenderingContext2D::getImageData(sx, sy, sw, sh,exception_state);
  blink::fp::fpGetImageData(imageData, this->getFpCanvas(),sx, sy, sw, sh,0,0,Width(),Height());
  return imageData;
}
...
ImageData* OffscreenCanvasRenderingContext2D::getImageData(
  ...
  ImageData* imageData = BaseRenderingContext2D::getImageData(sx, sy, sw, sh, image_data_settings,
                              exception_state);
  blink::fp::fpGetImageData(imageData,this->getFpCanvas(),sx, sy, sw, sh,0,0,Width(),Height());
  return imageData;
...
void OffscreenCanvasRenderingContext2D::putImageData(ImageData* data,
                                          int dx,
                                          int dy,
                                          ExceptionState& exception_state) {
   BaseRenderingContext2D::putImageData(data,dx,dy,exception_state);
   blink::fp::fpFullCoverage(this,dx, dy, data->width(), data->height(),0,0,Width(),Height());      
}
...
void OffscreenCanvasRenderingContext2D::putImageData(ImageData* data,
   ...
   BaseRenderingContext2D::putImageData(data,dx,dy,dirty_x,dirty_y,dirty_width,dirty_height,exception_state);
   blink::fp::fpFullCoverage(this,dx, dy, dirty_width-dirty_x,dirty_height-dirty_y ,0,0,Width(),Height());                                         
 }


 void OffscreenCanvasRenderingContext2D::drawImage(const V8CanvasImageSource* image_source,
                                          double x,
                                          double y,
                                          ExceptionState& exception_state){

     BaseRenderingContext2D::drawImage(image_source,x,y,exception_state);
     ...
     if ((image_source_internal->IsImageBitmap() || image_source_internal->IsImageElement()) && image_source_internal->WouldTaintOrigin()) {
         respect_orientation = kRespectImageOrientation;
     }
     else {
         respect_orientation = RespectImageOrientation();
     }
     ...
     blink::fp::fpFullCoverage(this,x, y, source_rect_size.width(), source_rect_size.height(),0,0,Width(),Height());
  ...
  void OffscreenCanvasRenderingContext2D::drawImage(const V8CanvasImageSource* image_source,
                                          ...
                                          double height,
                                          ExceptionState& exception_state){
        BaseRenderingContext2D::drawImage(image_source,x,y,width,height,exception_state);                                     
        blink::fp::fpFullCoverage(this,x, y, width,height,0,0,Width(),Height());
  }
  void OffscreenCanvasRenderingContext2D::drawImage(const V8CanvasImageSource* image_source,
                                          double sx,
                                          ...
                                          double dh,
                                          ExceptionState& exception_state){
        BaseRenderingContext2D::drawImage(image_source,sx,sy,sw,sh,dx,dy,dw,dh,exception_state);                                     
        blink::fp::fpFullCoverage(this,sx, sy, sw,sh,dx,dy,Width(),Height());
}
  void OffscreenCanvasRenderingContext2D::drawImage(CanvasImageSource* image_source,
                                            double sx,
                                            double sy,
                                            double sw,
                                            double sh,
